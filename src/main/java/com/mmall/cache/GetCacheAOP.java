...
import com.mmall.util.RedisCache;

@Component
@Aspect
public class GetCacheAOP  {

    private static  final Logger logger = LoggerFactory.getLogger(GetCacheAOP.class);
    @Autowired
    ...
    @Pointcut("@annotation(com.mmall.cache.GetCache)")
    ...
    private String getCacheKey(ProceedingJoinPoint joinPoint) {
        ...
        //设置在多久时间之后过期！
        String expireTime = method.getAnnotation(GetCache.class).value();
        //先获取目标方法参数
        String id = "";
        Object[] args = joinPoint.getArgs();
        if (args != null && args.length > 0) {
            for(int i=0;i<args.length;i++){
                id = id + ( i!=args.length-1 ? String.valueOf(args[i]) + "," : String.valueOf(args[i])) ;
            }
        }
        String redisKey = ms +"." + ActionName +"_"+ expireTime + " = " + id;
        return redisKey;
    }
